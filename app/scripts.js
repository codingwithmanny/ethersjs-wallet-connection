...
const onChainChanged = (chainId) => {
    ...
    if (!chainId) {
        ...
        const parsedChainId = `${chainId}`.startsWith('0x') ? parseInt(`${chainId}`, 16) : chainId;
...
const onSubmitContractWrite = async (event) => {
    ...
    try {
        ...
        preContractWrite.innerHTML = `${JSON.stringify(result, null, ' ')}\n\n// ${BLOCKCHAIN_EXPLORERS?.[CHAIN_CONNECTED?.id] ?? ''}/tx/${result?.hash ?? JSON.stringify(result)}`;
    } catch (error) {
        console.log({ error });
...
window.onload = async () => {
    ...
    if (typeof window?.ethereum !== "undefined") {
        ...
        // Events
        ETHERSJS_PROVIDER.provider.on('chainChanged', onChainChanged);
        ...
        if (hasWalletPermissions.length > 0 && shouldBeConnected) {
            // Retrieve chain
            // const chainId = await ETHERSJS_PROVIDER.send('eth_chainId')?.chainId;
            // OR
            const chainId = await ETHERSJS_PROVIDER.getNetwork()?.chainId;
            onChainChanged(chainId);
            connect();
        }
    }
